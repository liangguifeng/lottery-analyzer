...
/**
 * 杀百十和尾：预测数字之和尾数，一定不与百位、十位之和尾数相同.
 */
class KillHundredTenSumTailAnalyzer extends AbstractAnalyzer implements AnalyzerInterface
{
    /**
     * 百位十位路径标定.
     *
    ...
    private function checkCombinationsAgainstNext(array $combinations, array $nextData): array
    {
        ...
        foreach ($combinations as $key => $values) {
            ...
            $nextTail = array_sum(ArrayHelper::getByPath($nextData, $this->killPath)) % 10;
